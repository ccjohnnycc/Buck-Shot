...
import {
  View, Text, StyleSheet, ActivityIndicator, Button, ImageBackground,
  TextInput, TouchableOpacity, Image, Alert, Keyboard
} from 'react-native';
...
type ParsedPolygon = { id: string; coords: { latitude: number; longitude: number }[] };
type GeoJson = {
  features: {
...
export default function MapScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      // Reset transient UI on screen focus
      setFabOpen(false);
  ...
  // Parse & cache state-land polygons (lightly decimated)
  useEffect(() => {
    const parsed = (FloridaBoundariesSimplified as GeoJson).features.flatMap((feature, index) => {
      ...
      if (type === 'Polygon') {
        return [
          {
            id: `polygon-${index}`,
            coords: coordinates[0].slice(0, 200).map(([lng, lat]: [number, number]) => ({
              latitude: lat,
              longitude: lng,
            })),
          },
  ...
  // Save current map view as an image to local storage
  const saveMapSnapshot = async () => {
    if (!mapRef.current || savingDisabled) return;
    setSavingDisabled(true);
    try {
      const uri = await captureRef(mapRef.current, { format: 'jpg', quality: 0.9 });
      const folderUri = FileSystem.documentDirectory + 'offline_maps/';
      const destinationUri = folderUri + `map_${Date.now()}.jpg`;
    ...
    setTimeout(() => setSavingDisabled(false), 2000);
  };

  // Debounced: keep polygon set limited to current viewport
  const handleRegionChangeComplete = (newRegion: Region) => {
    setRegion(newRegion);
    if (debounceTimer.current) clearTimeout(debounceTimer.current);
    debounceTimer.current = setTimeout(() => filterPolygonsByRegion(newRegion), 500);
  };

  const getUpdatedRecentSearches = (newSearch: string, existing: string[]): string[] =>
    [...new Set([newSearch.trim(), ...existing])].slice(0, 5);
  ...
  // Online geocoding via Nominatim; caches recent queries
  const handleSearch = async () => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        setMarker({ latitude: searchRegion.latitude, longitude: searchRegion.longitude });
        mapRef.current?.animateToRegion(searchRegion, 1000);

        const existing = (await AsyncStorage.getItem('recentSearches')) || '[]';
        const updated = getUpdatedRecentSearches(search, JSON.parse(existing));
        await AsyncStorage.setItem('recentSearches', JSON.stringify(updated));
        setRecentSearches(updated);
      } else {
        setMarker(null);
        Alert.alert('No Results', 'No results found for that search. Try a different location.', [
          {
            text: 'Retry',
            onPress: () => {
              setSearch('');
  ...
  // Show only polygons that intersect current viewport (with small margin)
  const filterPolygonsByRegion = (r: Region) => {
    const margin = 0.5;
    ...
    if (r.latitudeDelta > 1.2 || r.longitudeDelta > 1.2) {
      setVisiblePolygons([]);
      return;
    }

    const filtered = parsedStateLandPolygons
      .filter((polygon) =>
        polygon.coords.some(
          (c) => c.latitude >= latMin && c.latitude <= latMax && c.longitude >= lonMin && c.longitude <= lonMax
        )
      )
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        // Center on FL by default; adjust polygons for viewport
        const defaultRegion = {
          latitude: 27.9944,
          longitude: -81.7603,
          latitudeDelta: 0.5,
  ...
  return (
    <ImageBackground source={require('../../assets/background_image.png')} style={{ flex: 1 }}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                if (text.trim().length === 0) {
                  ...
                  setSuggestions(
                    recentSearches.filter((s) => s.toLowerCase().includes(text.toLowerCase()))
                  );
                }
              }}
              onFocus={() => {
                if (search.trim() === '') setSuggestions(recentSearches);
              }}
      ...
      <View style={styles.fabContainer}>
        ...
        <TouchableOpacity style={styles.fabMainButton} onPress={() => setFabOpen((prev) => !prev)}>
          <Text style={styles.fabMainText}>{fabOpen ? 'Close Tools' : '⚙️ Map Tools'}</Text>
        </TouchableOpacity>
      </View>
      ...
      {activePin && (
        <View style={styles.deletePinOverlay}>
          ...
          <Button
            title="Save Title"
            onPress={async () => {
              const updatedPins = pins.map((p) => (p.id === activePin.id ? { ...p, title: activePin.title } : p));
              setPins(updatedPins);
          ...
          <Button
            ...
            color="#ff4444"
            onPress={() => {
              Alert.alert('Delete Pin', 'Are you sure you want to delete this item?', [
                { text: 'Cancel', style: 'cancel' },
                {
                  text: 'Delete',
                  style: 'destructive',
      ...
      {snapshotSaved && (
        <View style={styles.modal}>
          ...
          <Text style={{ color: '#fff', marginBottom: 12 }}>Map snapshot saved for offline viewing.</Text>
          <Button
            title="View Offline Maps"
            onPress={() => {
...
const styles = StyleSheet.create({
  ...
  searchContainer: { position: 'absolute', top: 54, left: 0, right: 0, paddingHorizontal: 18, zIndex: 20 },
  searchInput: {
    flex: 1,
    backgroundColor: '#222',
    color: '#fff',
  ...
  modal: { position: 'absolute', top: '30%', left: '10%', right: '10%', backgroundColor: '#333', padding: 20, borderRadius: 10, zIndex: 10 },
  ...
  sidebar: { position: 'absolute', right: 5, top: 120, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 10, paddingVertical: 6, paddingHorizontal: 6, zIndex: 10 },
  sidebarButton: { padding: 5, alignItems: 'center' },
  sidebarButtonActive: { backgroundColor: '#FFD700', borderRadius: 2 },
  sidebarIcon: { width: 50, height: 40, resizeMode: 'contain' },
  fabContainer: { position: 'absolute', bottom: 15, right: 15, flexDirection: 'column-reverse', alignItems: 'center', zIndex: 100 },
  fabAction: { backgroundColor: '#2f95dc', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 20, marginBottom: 10 },
  fabText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  fabMainButton: { backgroundColor: '#FFD700', paddingVertical: 10, paddingHorizontal: 15, borderRadius: 10, marginTop: 5, marginBottom: 7, alignItems: 'center', justifyContent: 'center', minWidth: 120 },
  fabMainText: { color: '#000', fontWeight: 'bold', fontSize: 14 },
});